...
def sort_by_lengths(word_lists, data_lists, wordlabel_lists, datalabel_lists, dataptr_lists):
    pairs = list(zip(word_lists, data_lists, wordlabel_lists, datalabel_lists, dataptr_lists))
    ...
    try:
        word_lists, data_lists, wordlabel_lists, datalabel_lists, dataptr_lists = list(zip(*pairs))
    except ValueError:
        print(wordlabel_lists)
        print(datalabel_lists)
        print(word_lists)
        
    return word_lists, data_lists, wordlabel_lists, datalabel_lists, dataptr_lists, indices
...
def cal_loss(logits, targets):
    ...
    targets = torch.stack([torch.tensor(x+[-1]*(maxlen-len(x))) for x in targets]).to(device)
    mask = (targets!=-1)
    targets = targets[mask]
    # print(targets)
    tmp = mask.unsqueeze(2).expand(-1, -1, logits.size()[2])
    # print(mask.size())
    # print(mask)
    # print(tmp.size())
    # print(tmp)
    ...
    logits = logits.masked_select(tmp).contiguous().view(-1, logits.size()[2])
