...
class BILSTM_Model(object):
    def __init__(self, vocab_size, data_size, out_size, crf=True):
        """功能：对LSTM的模型进行训练与测试
           参数:
            vocab_size:词典大小
            out_size:标注种类
        ...
        if not crf:
            ...
            self.model = BiLSTM_CRF(vocab_size, data_size, self.emb_size,
                                    self.hidden_size, out_size).to(self.device)
    ...
    def train(self, word_lists, data_lists, tag_lists,
              dev_word_lists, dev_data_lists, dev_tag_lists,
              word2id, data2id, tag2id):
        # 对数据集按照长度进行排序
        word_lists, data_lists, tag_lists, _ = sort_by_lengths(word_lists, data_lists, tag_lists)
        dev_word_lists, dev_data_lists, dev_tag_lists, _ = sort_by_lengths(
            dev_word_lists, dev_data_lists, dev_tag_lists)
        ...
        for e in range(1, self.epoches+1):
            ...
            for ind in range(0, len(word_lists), B):
                ...
                losses += self.train_step(batch_sents, batch_datas,
                                          batch_tags, word2id, data2id, tag2id)
            ...
            val_loss = self.validate(
                dev_word_lists, dev_data_lists, dev_tag_lists, word2id, data2id, tag2id)
            print("Epoch {}, Val Loss:{:.4f}".format(e, val_loss))

    def train_step(self, batch_sents, batch_datas, batch_tags, word2id, data2id, tag2id):
        self.model.train()
        self.step += 1
        # 准备数据
        ...
        # forward
        scores = self.model(tensorized_sents, tensorized_datas, lengths, lengthsD)
    ...
    def validate(self, dev_word_lists, dev_data_lists, dev_tag_lists, word2id, data2id, tag2id):
        self.model.eval()
        with torch.no_grad():
            val_losses = 0.
            val_step = 0
    ...
    def test(self, word_lists, data_lists, tag_lists, word2id, data2id, tag2id):
        """返回最佳模型在测试集上的预测结果"""
        # 准备数据
        word_lists, data_lists, tag_lists, indices = sort_by_lengths(word_lists, data_lists, tag_lists)
        ...
        with torch.no_grad():
            batch_tagids = self.best_model.test(
                tensorized_sents, tensorized_datas, lengths, lengthsD, tag2id)
...
class BiLSTM_CRF(nn.Module):
    def __init__(self, vocab_size, data_size, emb_size, hidden_size, out_size):
        ...
        self.bilstm = BiLSTM(vocab_size, data_size, emb_size, hidden_size, out_size)
    ...
    def forward(self, sents_tensor, data_tensor, lengths, lengthsD):
        # [B, L, out_size]
        emission = self.bilstm(sents_tensor, data_tensor, lengths, lengthsD)
    ...
    def test(self, test_sents_tensor, data_tensor, lengths, lengthsD, tag2id):
        """使用维特比算法进行解码"""
        ...
        crf_scores = self.forward(test_sents_tensor, data_tensor, lengths, lengthsD)
